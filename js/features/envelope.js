...
function envLetterTime(letter, section) {
    const raw = section === 'outbox'
        ? (letter.sentTime || letter.scheduleTime || letter.timestamp || letter.createdAt)
...
function generateEnvelopeReplyText() {
    ...
    if (sourcePool.length === 0) return '我认真读完了你的信，一时间有很多话想说。等我把心里的声音整理好，再慢慢写给你。';
    // 使用自定义设置或默认值
...
function renderOutboxList() {
    ...
    list.innerHTML = envelopeData.outbox.slice().reverse().map(letter => {
        ...
        const origPreview = originalContent ? (String(originalContent).length > 32 ? String(originalContent).substring(0, 32) + '…' : String(originalContent)) : '';
...
function renderSpacetimeList() {
    ...
    list.innerHTML = envelopeData.spacetime.slice().reverse().map(letter => {
        const date = new Date(envLetterTime(letter, 'spacetime')).toLocaleDateString('zh-CN', {month:'numeric', day:'numeric', hour:'2-digit', minute:'2-digit'});
        const contentText = String(letter.content || '');
        const preview = contentText.length > 50 ? contentText.substring(0, 50) + '…' : contentText;
        const safePreview = envEscapeHtml(preview);
        const isNew = letter.isNew;
...
window.viewEnvLetter = function(section, id) {
    ...
    if (typeof window.ensureEnvelopeViewStack === 'function') window.ensureEnvelopeViewStack();
};
