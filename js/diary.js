...
function showMiniCalDayDetail(dateStr) {
    ...
    if (diaryPeriodRecords.some(r => r.date === dateStr)) {
        ...
        for (let i = 0; i < sortedRecords.length; i++) {
            ...
            const prevStr = formatDiaryDateStr(prev);
...
function renderDiaryTodos() {
    ...
    const categoryLabel = diaryTodoSubTab !== 'all' ? `「${getDiaryTodoCategoryName(diaryTodoSubTab)}」分类` : '';
...
// 获取分类名称
function getDiaryTodoCategoryName(categoryId) {
...
function renderDiaryPeriod() {
    ...
    if (lastPeriodStart && avgCycle > 0) {
        ...
        predictedStartStr = formatDiaryDateStr(startDate);
        ...
        if (currentMonthRecords.length > 0) {
            ...
            ovulationDates.add(formatDiaryDateStr(ovulationDay));

            // 排卵期 = 排卵日前后各5天
            for (let j = -5; j <= 5; j++) {
                const od = new Date(ovulationDay);
                od.setDate(od.getDate() + j);
                ovulationPeriodDates.add(formatDiaryDateStr(od));
            }
            ...
            while (predictDate <= maxDate) {
                for (let i = 0; i < dur; i++) {
                    ...
                    d.setDate(d.getDate() + i);
                    predictedDates.add(formatDiaryDateStr(d));
                }
                const ovDay = new Date(predictDate);
                ovDay.setDate(ovDay.getDate() - 14);
                ovulationDates.add(formatDiaryDateStr(ovDay));
                for (let j = -5; j <= 5; j++) {
...
function fillDateRange(dateSet, startDate, endDate) {
    ...
    while (current <= end) {
        dateSet.add(formatDiaryDateStr(current));
...
// 格式化日期为 YYYY-MM-DD
function formatDiaryDateStr(date) {
...
function checkPeriodReminder() {
    ...
    console.log('[朝夕心记] 经期提醒检查: 今天=' + todayStr + ', 下次经期=' + formatDiaryDateStr(nextPeriodDate) + ', 距离=' + diffDays + '天, 今天是经期=' + todayIsPeriod);
    ...
    if (todayIsPeriod) {
        ...
        const yesterdayStr = formatDiaryDateStr(yesterday);
