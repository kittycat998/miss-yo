...
function renderFavorites() {
    ...
    list.innerHTML = favoritedMessages.map(msg => {
        ...
        if (msg.type === 'share' && msg.shareData) {
            ...
            const cents = Number(msg.redPacket.amount || 0);
            const amount = (cents / 100).toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
            const note = msg.redPacket.message || msg.text || '';
            content = `[红包信息:]¥${amount}${note ? ' ' + note.replace(/</g, '&lt;').replace(/>/g, '&gt;') : ''}`;
        } else if (msg.text) {
    ...
    list.querySelectorAll('.fav-remove-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            const msg = (typeof messages !== 'undefined' ? messages : []).find(m => String(m.id) === String(id));
            if (msg) {
                msg.favorited = false;
...
function initComboMenu() {
    ...
    function normalizeStickerEntry(entry) {
        if (typeof entry === 'string') return { name: '', url: entry };
        if (entry && typeof entry === 'object') {
            return {
                name: String(entry.name || entry.label || entry.title || '').trim(),
                url: String(entry.url || entry.src || entry.image || entry.data || '').trim()
            };
        }
        return { name: '', url: '' };
    }
    ...
    function switchTab(tabId) {
        ...
        const urlBtn = document.getElementById('sticker-url-add-btn');
        if (urlBtn) urlBtn.style.display = (tabId === 'my-sticker' || tabId === 'partner-sticker') ? 'flex' : 'none';
    ...
    function makeStickerItem(src, onClick, name) {
        const item = document.createElement('div');
        item.className = 'sticker-grid-item' + (name ? ' has-caption' : '');
        item.innerHTML = `<img src="${src}" loading="lazy">${name ? `<div class="sticker-caption">${escapeHtml(name)}</div>` : ''}`;
        item.onclick = (e) => { e.stopPropagation(); onClick(); };
        return item;
    }

    function makeDeletableStickerItem(sticker, onClick, onDelete) {
        const info = normalizeStickerEntry(sticker);
        const item = document.createElement('div');
        item.className = 'sticker-grid-item' + (info.name ? ' has-caption' : '');
        item.style.position = 'relative';
        item.innerHTML = `<img src="${info.url}" loading="lazy">${info.name ? `<div class="sticker-caption">${escapeHtml(info.name)}</div>` : ''}<div class="sticker-delete-btn" title="删除"><i class="fas fa-times"></i></div>`;
        item.querySelector('img').onclick = (e) => { e.stopPropagation(); onClick(info); };
        const cap = item.querySelector('.sticker-caption');
        if (cap) cap.onclick = (e) => { e.stopPropagation(); onClick(info); };
    ...
    function renderMyStickerLibrary() {
        ...
        if (!myStickerLibrary || myStickerLibrary.length === 0) {
            contentArea.innerHTML = `
                <div class="empty-sticker-tip">
                    ...
                    还没有我的专属表情哦<br>
                    点击右上角“添加”上传，或点“URL”批量添加：大笑:https://xxx.jpg
                </div>
            `;
            return;
        }
        ...
        toolbar.className = 'sticker-search-toolbar';
        toolbar.innerHTML = `<i class="fas fa-search"></i><input id="my-sticker-search-input" type="text" placeholder="搜索表情释义/关键词，比如：大笑、委屈、亲亲">`;
        contentArea.appendChild(toolbar);

        const grid = document.createElement('div');
        grid.className = 'sticker-grid-view sticker-grid-view-named';
        contentArea.appendChild(grid);

        const renderFiltered = () => {
            const keyword = (toolbar.querySelector('input').value || '').trim().toLowerCase();
            grid.innerHTML = '';
            let shown = 0;
            myStickerLibrary.forEach((entry, idx) => {
                const info = normalizeStickerEntry(entry);
                if (!info.url) return;
                const hay = (info.name + ' ' + info.url).toLowerCase();
                if (keyword && !hay.includes(keyword)) return;
                shown++;
                const item = makeDeletableStickerItem(entry, (picked) => {
                    addMessage({ id: Date.now(), sender: 'user', text: '', timestamp: new Date(), image: picked.url, status: 'sent', type: 'normal' });
                    playSound('send');
                    picker.classList.remove('active');
            ...
            if (!shown) grid.innerHTML = '<div class="empty-sticker-tip sticker-empty-inline"><i class="fas fa-search"></i> 没找到匹配的表情</div>';
        };
        toolbar.querySelector('input').addEventListener('input', renderFiltered);
        renderFiltered();
    }
    ...
    function renderPartnerStickerLibrary() {
        ...
        const toolbar = document.createElement('div');
        toolbar.className = 'sticker-search-toolbar';
        toolbar.innerHTML = `<i class="fas fa-search"></i><input id="partner-sticker-search-input" type="text" placeholder="搜索对方表情释义/关键词">`;
        contentArea.appendChild(toolbar);
        ...
        const renderFiltered = () => {
            ...
            grid.innerHTML = '';
            let shown = 0;
            stickerLibrary.forEach(entry => {
                const info = normalizeStickerEntry(entry);
                if (!info.url) return;
                const hay = (info.name + ' ' + info.url).toLowerCase();
                if (keyword && !hay.includes(keyword)) return;
                shown++;
                const item = makeStickerItem(info.url, () => {
                    addMessage({ id: Date.now(), sender: 'user', text: '', timestamp: new Date(), image: info.url, status: 'sent', type: 'normal' });
                    playSound('send');
                    picker.classList.remove('active');
                    const delayRange = settings.replyDelayMax - settings.replyDelayMin;
                    setTimeout(simulateReply, settings.replyDelayMin + Math.random() * delayRange);
            ...
            if (!shown) grid.innerHTML = '<div class="empty-sticker-tip sticker-empty-inline"><i class="fas fa-search"></i> 没找到匹配的对方表情</div>';
        };
        toolbar.querySelector('input').addEventListener('input', renderFiltered);
        renderFiltered();
    }
    ...
    window.renderMyStickerLibrary = renderMyStickerLibrary;
...
// 兜底：表情包菜单不能依赖整套 setupEventListeners 全部成功。
// 只要 DOM 和 initComboMenu 都在，就单独补绑一次。
(function ensureComboMenuAutoInit(){
    function run(){
        try {
            if (typeof initComboMenu === 'function') initComboMenu();
        } catch (e) {
            console.warn('[combo] init failed:', e);
        }
    }
