...
(function() {
    ...
    const homeGlobalLargeCache = {};
    const homeGlobalLargePending = {};
    ...
    /** 读取全局 Home 设置（不受开关影响）：优先 localStorage，若大图只在 localforage，则异步补进缓存 */
    function homeGetGlobal(key) {
        let direct = null;
        try { direct = localStorage.getItem(key); } catch(e) { direct = null; }
        if (direct !== null) {
            homeGlobalLargeCache[key] = direct;
            return direct;
        }
        if (Object.prototype.hasOwnProperty.call(homeGlobalLargeCache, key)) {
            return homeGlobalLargeCache[key];
        }
        if (typeof localforage !== 'undefined' && !homeGlobalLargePending[key]) {
            homeGlobalLargePending[key] = true;
            localforage.getItem(key).then(value => {
                if (value !== null && value !== undefined) {
                    homeGlobalLargeCache[key] = value;
                    window.dispatchEvent(new CustomEvent('homeGlobalUpdated', { detail: { key, value } }));
                }
            }).catch(() => {}).finally(() => { homeGlobalLargePending[key] = false; });
        }
        return null;
    }

    async function homeGetGlobalLarge(key) {
        // 大图/头像优先读 localforage，避免 localStorage 里残留旧图或写爆后的旧值盖住新值。
        if (typeof localforage !== 'undefined') {
            try {
        ...
        const direct = homeGetGlobal(key);
    ...
    function homeSetGlobal(key, value) {
        homeGlobalLargeCache[key] = value;
        // 同时写入 localStorage 和 localforage，确保各模块都能读取；localStorage 可能因大图超限失败，localforage 才是兜底。
        try {
            localStorage.setItem(key, value);
        } catch(e) {
            // 写大图失败时清掉旧 localStorage 值，防止下次刷新优先读到过期/裂图数据。
            try { localStorage.removeItem(key); } catch(_) {}
            console.warn('[Home] localStorage 全局写入失败，已保留 localforage/内存缓存:', key, e);
        }
        if (typeof localforage !== 'undefined') {
    ...
    window.homeGetGlobalLarge = homeGetGlobalLarge;
    ...
    function homeRemoveGlobal(key) {
        ...
        delete homeGlobalLargeCache[key];
        if (typeof localforage !== 'undefined') {
    ...
    const defaultAppIcons = {
        ...
        'ta-phone': '<i class="fas fa-mobile-screen-button"></i>',
        map: '<i class="fas fa-map-marked-alt"></i>',
        'music-game': '<i class="fas fa-music"></i>'
    };

    const defaultAppOrder = ['shop', 'chat', 'mailbox', 'moyu', 'diary', 'fortune', 'mood', 'calendar', 'decide', 'gift-cabinet', 'stats', 'accounting', 'pet', 'ta-phone', 'map', 'music-game'];
    ...
    window.handlePageBgUpload = function(event) {
        ...
        reader.onload = async function(e) {
            ...
            try {
                ...
                console.warn('[Home] 页面背景保存失败:', err);
                (window.showNotification || function(){})('页面背景保存失败，图片可能太大', 'warning');
            }
    ...
    window.syncChatBgToHome = function(bgValue) {
        ...
        if (!bgValue) {
            ...
            // 保存为自定义背景：背景可能是 dataURL，必须走大容量存储
            let rawBgValue = bgValue;
            const bgMatch = String(bgValue).match(/url\(["']?([^"')]+)["']?\)/);
            if (bgMatch && bgMatch[1]) rawBgValue = bgMatch[1];
            homeSetLargeItem('home_page_bg_custom', rawBgValue).catch(e => console.warn('[Home] 同步页面背景保存失败:', e));
            homeSetItem('home_page_bg', 'custom');
        }
    };
    ...
    function renderIconGrid() {
        ...
        grid.innerHTML = '';
        const nameMap = { shop:'商城', chat:'聊天', mailbox:'信封', moyu:'摸鱼', diary:'朝夕心记', fortune:'运势', mood:'心晴', calendar:'日历', decide:'抉择', 'gift-cabinet':'礼物柜', stats:'统计', accounting:'记账', pet:'萌宠屋', 'ta-phone':'TA的手机', map:'Zmilk地图', 'music-game':'音游' };
    ...
    function updateHeroTitleFromProfiles() {
        const heroTitle = document.getElementById('hero-title');
        if (heroTitle) {
            heroTitle.textContent = `${profileData.partner.name} & ${profileData.me.name}`;
        }
    }
    ...
    window.openApp = function(app) {
        const appActions = {
            ...
            'gift-cabinet': () => {
                if (window.GiftCabinetApp && typeof window.GiftCabinetApp.open === 'function') {
                    window.GiftCabinetApp.open();
                } else {
                    (window.showNotification || function(){})('礼物柜加载失败，请刷新重试', 'error');
                }
            },
            'shop': () => {
            ...
            'music-game': () => {
                if (window.MusicGameApp && typeof window.MusicGameApp.open === 'function') {
                    window.MusicGameApp.open();
                } else {
                    (window.showNotification || function(){})('音游加载失败，请刷新重试', 'error');
                }
            },
            'moments': () => {
                // 朋友圈已经在 index.html/js/moments.js 内联合并，直接走现有页面切换，
                // 不再 fetch 不存在的 moments.html，避免入口被补上后点击时报 404/无响应。
                if (typeof window.showMomentsPage === 'function') {
                    window.showMomentsPage();
                } else if (typeof window.MomentsApp !== 'undefined' && window.MomentsApp.init) {
                    window.MomentsApp.init();
                    const momentsContainer = document.getElementById('moments-container');
                    if (momentsContainer) momentsContainer.style.display = 'block';
                } else {
                    (window.showNotification || function(){})('朋友圈加载失败，请刷新重试', 'error');
                }
            }
        };
    ...
    async function loadSavedSettings() {
        ...
        if (savedPageBg === 'custom') {
            ...
            if (customUrl) {
                const pageBg = document.getElementById('home-page-bg');
                if (pageBg) {
                    const bgValue = /^url\(/.test(customUrl) || /gradient\(/.test(customUrl)
                        ? customUrl
