                ...
                if (isBatchFavoriteMode || window.isScreenshotSelectMode) {
                    ...
                    if (wrapper && !e.target.closest('.message-meta-actions') && !e.target.closest('.call-event-delete')) {
                        ...
                        if (messageId) {
                            ...
                            if (window.isScreenshotSelectMode && typeof window.updateScreenshotSelectCount === 'function') {
                                window.updateScreenshotSelectCount();
                            } else if (typeof updateBatchFavoriteActionCounts === 'function') {
                                ...
                                if (saveSelectedBtn) saveSelectedBtn.innerHTML = `<i class="fas fa-download"></i> 保存所选 (${selectedMessages.length})`;
                            }
                        }
                        return;
                    }
...
if (_chatSettingsEl) _chatSettingsEl.addEventListener('click', () => {
    ...
    setupAvatarFrameSettings();
            ...
            if (_dataSettingsEl) _dataSettingsEl.addEventListener('click', () => {
                ...
                if (typeof updateStorageUsageBar === 'function') {
                    setTimeout(updateStorageUsageBar, 80);
                }
            });
            ...
            document.querySelectorAll('[data-bubble-style]').forEach(item => {
                item.addEventListener('click',
                    () => {
                        window.__bubbleStyleChangedThisSession = true;
...
if (_scheduleSendCheckbox && _scheduleTimeContainer) {
    _scheduleSendCheckbox.addEventListener('change', () => {
        ...
        if (_scheduleSendCheckbox.checked && _scheduleTimeInput) {
            ...
            const _screenshotSelectEl = document.getElementById('screenshot-select-function');
            if (_screenshotSelectEl) _screenshotSelectEl.addEventListener('click', () => {
                hideModal(DOMElements.favoritesModal.modal);
                if (typeof toggleScreenshotSelectMode === 'function') toggleScreenshotSelectMode();
            });
