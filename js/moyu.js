...
// 兜底初始化：之前只定义了 initMoyu，但没有稳定调用，导致顶部“摸鱼小记”入口点不动。
(function ensureMoyuAutoInit(){
    function run(){
        if (window.__moyuAutoInitDone) return;
        window.__moyuAutoInitDone = true;
        try { if (typeof window.initMoyu === 'function') window.initMoyu(); } catch (e) { console.warn('[moyu] init failed:', e); }
    }
